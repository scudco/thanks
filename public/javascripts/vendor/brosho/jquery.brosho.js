...
(function($) {
  $.fn.brosho = function(options) {
    ...
    if($('#brosho-wrapper').length) { //brosho got called the second time by user, remove the current and add back with the options
      ...
      $.getScript('/javascripts/vendor/brosho/jquery.autocomplete.js'); //load autocomplete plugin
      $.getScript('/javascripts/vendor/brosho/jquery.caret.js'); //load caret plugin
    }
  ...
  $.fn.brosho.defaults = {
    stylesheet:         '/javascripts/vendor/brosho/jquery.brosho.css',
    position:           'bottom',
